      ...
      <aside className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} sm:translate-x-0`}>
        <div className="h-full px-3 py-5 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          ...
          <ul className="space-y-2 font-medium">
            <li>
              <Link href="/" className={`flex items-center p-2 rounded-lg ${isActive("/") ? "bg-Blue_NFED_2 text-white" : "bg-white text-black hover:bg-gray-100 hover:text-black"} transition`}>
                <HomeIcon className={`${isActive("/") ? "text-white" : "text-gray-600"}`} />
                <span className="ms-3">Overview</span>
              </Link>
            </li>
            <li>
              <Link href="/powerMonitoring" className={`flex items-center p-2 rounded-lg ${isActive("/powerMonitoring") ? "bg-Blue_NFED_2 text-white" : "bg-white text-black hover:bg-gray-100 hover:text-black"} transition`}>
                <BoltIcon className={`${isActive("/powerMonitoring") ? "text-white" : "text-gray-600"}`} />
                <span className="ms-3">Power Monitoring</span>
              </Link>
            </li>
            <li>
              <Link href="/weatherStation" className={`flex items-center p-2 rounded-lg ${isActive("/weatherStation") ? "bg-Blue_NFED_2 text-white" : "bg-white text-black hover:bg-gray-100 hover:text-black"} transition`}>
